...
public sealed class Board : MonoBehaviour
{
    ...
    {
        // Если ячейка уже выбрана — игнорируем повторный выбор
        if (_selection.Contains(tile)) return;

        // Если есть уже выбранная ячейка
        if (_selection.Count > 0)
        {
            var firstTile = _selection[0];

            // Проверяем, является ли текущая клетка соседом
            bool isNeighbour = Array.IndexOf(firstTile.Neighbours, tile) != -1;

            if (isNeighbour)
            {
                // Добавляем второго соседа и пробуем свапнуть
                _selection.Add(tile);
            }
            else
            {
                // Если это не сосед — сбрасываем выбор и выбираем новую
                _selection.Clear();
                _selection.Add(tile);
                return;
            }
        ...
        {
            // Если ничего не выбрано — просто добавляем
            _selection.Add(tile);
            return;
        }
